...
public interface IUsageEventService
{
    ...
    /// **Pagination**: Navigate through large result sets</para>
    ///
    ...
    /// filter criteria (if any) - If you also provide `event_name`, it must match
    /// the meter's event name</para>
    ///
    /// <para>## Example Queries: - Get all events for a customer: `?customer_id=cus_abc123`
    /// - Get API request events: `?event_name=api_request` - Get events from last
    ...
    /// requests with existing `event_id` values are ignored (idempotent behavior)
    /// - **Rate Limiting**: Maximum 1000 events per request - **Time Validation**:
    /// Events with timestamps older than 1 hour or more than 5 minutes in the future
    /// will be rejected - **Metadata Limits**: Maximum 50 key-value pairs per event,
    /// keys max 100 chars, values max 500 chars</para>
    ///
    /// <para>## Example Usage: ```json {   "events": [     {       "event_id": "api_call_12345",
    ///       "customer_id": "cus_abc123",       "event_name": "api_request",
    ///    "timestamp": "2024-01-15T10:30:00Z",       "metadata": {         "endpoint":
...
public interface IUsageEventServiceWithRawResponse
{
    ...
    /// <summary>
    /// Returns a raw HTTP response for `post /events/ingest`, but is otherwise the
