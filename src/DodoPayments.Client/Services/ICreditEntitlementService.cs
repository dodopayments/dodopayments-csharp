...
public interface ICreditEntitlementService
{
    ...
    /// "Tokens", "Credits") - `expires_after_days` - Number of days after which credits
    /// expire (optional) - `rollover_enabled` - Whether unused credits can rollover to
    /// the next period - `rollover_percentage` - Percentage of unused credits that
    /// rollover (0-100) - `rollover_timeframe_count` - Count of timeframe periods for
    /// rollover limit - `rollover_timeframe_interval` - Interval type (day, week,
    /// month, year) - `max_rollover_count` - Maximum number of times credits can be
    /// rolled over - `overage_enabled` - Whether overage charges apply when credits run
    /// out (requires price_per_unit) - `overage_limit` - Maximum overage units allowed
    /// (optional) - `currency` - Currency for pricing (required if price_per_unit is
    /// set) - `price_per_unit` - Price per credit unit (decimal)</para>
    ///
    ...
    /// required when overage_enabled is true - rollover_timeframe_count and
    /// rollover_timeframe_interval must both be set or both be null</para>
    /// </summary>
    ...
    /// authenticated business - `500 Internal Server Error` - Database or server error</para>
    ///
    ...
    /// business (business_id check) - Deleted entitlements return a 404 error and must
    /// be retrieved via the list endpoint with `deleted=true`</para>
    /// </summary>
    ...
    ///
    /// <para># Request Body (all fields optional) - `name` - Human-readable name of the
    /// credit entitlement (1-255 characters) - `description` - Optional description
    /// (max 1000 characters) - `unit` - Unit of measurement for the credit (1-50
    /// characters)</para>
    ///
    ...
    /// credits can rollover to the next period - `rollover_percentage` - Percentage of
    /// unused credits that rollover (0-100, nullable) - `rollover_timeframe_count` -
    /// Count of timeframe periods for rollover limit (nullable) -
    ...
    /// (nullable) - `overage_enabled` - Whether overage charges apply when credits run
    /// out - `overage_limit` - Maximum overage units allowed (nullable) - `currency` -
    /// Currency for pricing (nullable) - `price_per_unit` - Price per credit unit
    /// (decimal, nullable)</para>
    ///
    ...
    /// successful modification - Changes take effect immediately but do not
    /// retroactively affect existing credit grants - The merged state is validated:
    /// currency required with price, rollover timeframe fields together, price required
    /// for overage</para>
    /// </summary>
    Task Update(
    ...
    /// Boolean flag to list deleted entitlements instead of active ones (default:
    /// false)</para>
    ///
    ...
    /// the entitlement becomes immediately available in the standard list and get
    /// endpoints - All configuration settings are preserved during delete/restore
    /// operations</para>
    ///
...
public interface ICreditEntitlementServiceWithRawResponse
{
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /credit-entitlements</c>, but is otherwise the
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>get /credit-entitlements/{id}</c>, but is otherwise the
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>patch /credit-entitlements/{id}</c>, but is otherwise the
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>get /credit-entitlements</c>, but is otherwise the
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>delete /credit-entitlements/{id}</c>, but is otherwise the
    ...
    /// <summary>
    /// Returns a raw HTTP response for <c>post /credit-entitlements/{id}/undelete</c>, but is otherwise the
