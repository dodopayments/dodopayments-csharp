...
public record class SubscriptionUpdateParams : ParamsBase
{
    ...
    /// on the invoice instead of the customer's personal name. Send `null` to explicitly
    ...
    ///
    /// <para>Set to `paused` to pause an active subscription. Set to `active` to
    /// resume a `paused` subscription. `active` also resumes an `on_hold` subscription
    /// that has an unpaid pause invoice. This voids that invoice.</para>
    ///
    /// <para>Send `paused` or `active` alone. A request that combines either with
    /// any other field fails with 422. `cancelled` is not exclusive this way — see
    /// `cancel_reason` and friends below.</para>
    /// </summary>
