...
public record class GroupCreateParams : ParamsBase
{
    ...
    public required IReadOnlyList<GroupProduct> Products
    {
        get
        {
            ...
            return this._rawBodyData.GetNotNullStruct<ImmutableArray<GroupProduct>>("products");
        }
        init
        {
            this._rawBodyData.Set<ImmutableArray<GroupProduct>>(
                "products",
...
#pragma warning restore CS8618
    ...
    public virtual bool Equals(GroupCreateParams? other)
    {
        if (other == null)
        {
            return false;
    ...
    public override Uri Url(ClientOptions options)
    {
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/')
                + string.Format("/product-collections/{0}/groups", this.ID)
        )
        {
