...
using Dodopayments.Models.UsageEvents;

namespace Dodopayments.Services.UsageEvents;
...
public interface IUsageEventService
{
    ...
    ///
    /// ## Event ID Format: The event ID should be the same value that was provided
    ...
    /// metadata key-value pairs - Original timestamp (preserved from ingestion)
    /// - Customer and business association - Event name and processing information
    ///
    ...
    ///
    /// ## Filtering Options: - **Customer filtering**: Filter by specific customer
    /// ID - **Event name filtering**: Filter by event type/name - **Meter-based
    /// filtering**: Use a meter ID to apply the meter's event name and filter criteria
    /// automatically - **Time range filtering**: Filter events within a specific
    /// date range - **Pagination**: Navigate through large result sets
    ///
    /// ## Meter Integration: When using `meter_id`, the endpoint automatically applies:
    /// - The meter's configured `event_name` filter - The meter's custom filter
    ...
    /// </summary>
    Task<UsageEventListPageResponse> List(UsageEventListParams parameters);
    ...
    ///
    /// ## Important Notes: - **Duplicate Prevention**:   - Duplicate `event_id`
    ...
    /// keys max 100 chars, values max 500 chars
    ///
